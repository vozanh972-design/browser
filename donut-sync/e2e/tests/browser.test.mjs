...
/// Read every prfnoir-proxy worker config the app has on disk.
...
// prfnoir-proxy has to notice its browser is gone and exit on its own — before it
...
test("a proxy worker dies with its browser, with and without the app running", async () => {
  ...
  try {
    ...
    await waitForCondition(
      ...
      `prfnoir-proxy ${first.workerPid} to exit after its browser did`,
    );
    assert.equal(
      (await readWorkerConfigs(app)).find(
    ...
    await waitForCondition(
      ...
      `orphaned prfnoir-proxy ${second.workerPid} to reap itself with no app running`,
    );
  } finally {
