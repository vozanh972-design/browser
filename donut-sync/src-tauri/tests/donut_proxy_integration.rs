...
/// Setup function to ensure prfnoir-proxy binary exists and cleanup stale proxies
async fn setup_test() -> Result<std::path::PathBuf, Box<dyn std::error::Error + Send + Sync>> {
  ...
  // Build prfnoir-proxy binary if it doesn't exist
      ...
      .is_ok_and(|output| {
        output.status.success()
          && String::from_utf8_lossy(&output.stdout).trim()
            == format!("prfnoir-proxy {}", env!("BUILD_VERSION"))
      });

  if !binary_is_current {
    println!("Building prfnoir-proxy binary for integration tests...");
...
async fn test_bypass_rules_http_direct() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  // Start prfnoir-proxy with upstream + bypass rules for "127.0.0.1"
...
/// Test that a SOCKS5 upstream proxy works end-to-end through prfnoir-proxy.
/// Starts a mock SOCKS5 server, a mock HTTP target server,
/// then routes requests through prfnoir-proxy -> SOCKS5 -> target.
#[tokio::test]
#[serial]
async fn test_local_proxy_with_socks5_upstream(
  ...
  // Test 1: HTTP request through prfnoir-proxy -> SOCKS5 -> target
...
/// Verifies the full chain: client → prfnoir-proxy → Shadowsocks → internet.
#[tokio::test]
#[serial]
async fn test_local_proxy_with_shadowsocks_upstream(
  ...
  // Start prfnoir-proxy with Shadowsocks upstream
  ...
  // Test: HTTP request through prfnoir-proxy → Shadowsocks → example.com
  let mut stream = TcpStream::connect(("127.0.0.1", local_port)).await?;
  let request =
...
// ---------------------------------------------------------------------------
// Worker lifecycle: a detached prfnoir-proxy must die with the browser it serves,
...
/// prfnoir-proxy after closing the browser".
#[tokio::test]
#[serial]
