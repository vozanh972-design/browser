...
async fn ensure_prfnoir_proxy_binary() -> Result<PathBuf, Box<dyn std::error::Error + Send + Sync>> {
...
async fn run_proxy_feature_suite(
  ...
  assert!(
    ...
    "HTTP traffic through prfnoir-proxy+VPN tunnel should succeed, got: {}",
  ...
  assert!(
    ...
    "Bypass rules should still work when prfnoir-proxy is chained to a VPN worker"
  );
...
#[tokio::test]
#[serial]
async fn test_wireguard_traffic_flows_through_prfnoir_proxy(
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  let binary_path = ensure_prfnoir_proxy_binary().await?;
