...
impl AppAutoUpdater {
  ...
  async fn prepare_windows_installer() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let remaining: Vec<u32> = worker_pids
      ...
      .collect();
    log::error!(
      "App update aborted because prfnoir-proxy worker PIDs are still running: {:?}",
      remaining
    );
    Err(
      serde_json::json!({
...
";
  ...
  fn test_windows_installer_hook_protects_sidecar_replacement() {
    ...
    assert!(hooks.contains("IfFileExists \"$INSTDIR\\prfnoir-proxy.exe\""));
    assert!(hooks.contains("taskkill.exe"));
    assert!(hooks.contains("prfnoir-proxy.exe"));
    assert!(hooks.contains("Delete \"$INSTDIR\\prfnoir-proxy.exe\""));
  }
