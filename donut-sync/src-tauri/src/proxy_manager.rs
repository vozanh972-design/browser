...
impl ProxyManager {
  ...
  // Check if a proxy is valid by routing through a temporary prfnoir-proxy process.
  ...
  pub async fn start_proxy(
    ...
    // Start a new proxy using the prfnoir-proxy binary with the correct CLI interface
    let mut proxy_cmd = app_handle
      .shell()
      .sidecar("prfnoir-proxy")
    ...
    // The prfnoir-proxy binary should start the worker and then exit
    let output = proxy_cmd
      .output()
      .await
      .map_err(|e| format!("Failed to execute prfnoir-proxy: {e}"))?;
  ...
  pub async fn stop_proxy(
    ...
    // Stop the proxy using the prfnoir-proxy binary
    ...
    match proxy_cmd.output().await {
      ...
      Ok(_) => {}
      Err(e) => log::warn!("Failed to run prfnoir-proxy stop: {e}"),
    }
  ...
  pub async fn cleanup_dead_proxies(
    ...
    // prfnoir-proxy processes accumulating this way.
    {
...
mod tests {
  ...
  // Helper function to build prfnoir-proxy binary for testing
  async fn ensure_prfnoir_proxy_binary() -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    // Build the prfnoir-proxy binary
    println!("Building prfnoir-proxy binary for tests...");

    let build_status = Command::new("cargo")
      .args(["build", "--bin", "prfnoir-proxy"])
      .current_dir(project_root.join("src-tauri"))
      .status()
      .await?;

    if !build_status.success() {
      return Err("Failed to build prfnoir-proxy binary".into());
    }

    if !proxy_binary.exists() {
      return Err("prfnoir-proxy binary was not created successfully".into());
    }
  ...
  // Integration test that actually builds and uses prfnoir-proxy binary
  #[tokio::test]
  async fn test_proxy_integration_with_real_proxy() -> Result<(), Box<dyn std::error::Error>> {
    // This test requires prfnoir-proxy binary to be available
  ...
  async fn test_cli_exits_after_proxy_start() -> Result<(), Box<dyn std::error::Error>> {
    let proxy_path = ensure_prfnoir_proxy_binary().await?;
