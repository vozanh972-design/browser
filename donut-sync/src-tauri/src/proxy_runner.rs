...
pub(crate) async fn ensure_sidecar_version() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let executable = match find_sidecar_executable("prfnoir-proxy") {
    Ok(executable) => executable,
    Err(e) => {
      log::error!("Failed to locate prfnoir-proxy for version verification: {e}");
  ...
  log::error!(
    "prfnoir-proxy version mismatch: expected {}, got {:?}; status={}, stdout={:?}, stderr={:?}",
    expected_version,
    actual_version,
    output.status,
...
pub async fn start_proxy_process_with_profile(
  ...
  let exe = find_sidecar_executable("prfnoir-proxy")?;
  let temp_dir = std::env::temp_dir();
  let log_path = temp_dir.join(format!("prfnoir-proxy-{id}.log"));
...
mod tests {
  ...
  fn parses_exact_sidecar_version_output() {
    assert_eq!(
      parse_sidecar_version(b"prfnoir-proxy v0.28.2\n").as_deref(),
      Some("v0.28.2")
    );
    assert_eq!(
      parse_sidecar_version(b"prfnoir-proxy nightly-2026-07-19-a4ed5c8\r\n").as_deref(),
  ...
  fn rejects_missing_or_ambiguous_sidecar_version_output() {
    ...
    assert_eq!(parse_sidecar_version(b"prfnoir-proxy"), None);
  ...
  fn prunes_only_old_proxy_logs() {
    ...
    for id in ["oldest", "middle", "newest"] {
      fs::write(temp.path().join(format!("prfnoir-proxy-{id}.log")), id).unwrap();
    ...
    assert!(!temp.path().join("prfnoir-proxy-oldest.log").exists());
    assert!(temp.path().join("prfnoir-proxy-middle.log").exists());
    assert!(temp.path().join("prfnoir-proxy-newest.log").exists());
