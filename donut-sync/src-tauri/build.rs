fn main() {
  ...
  // This allows building prfnoir-proxy sidecar without the other binaries present
...
fn external_binaries_exist() -> bool {
  ...
  let prfnoir_proxy_name = if target.contains("windows") {
    format!("prfnoir-proxy-{}.exe", target)
  } else {
    format!("prfnoir-proxy-{}", target)
  };
  ...
  binaries_dir.join(&prfnoir_proxy_name).exists() && binaries_dir.join(&xray_name).exists()
}
